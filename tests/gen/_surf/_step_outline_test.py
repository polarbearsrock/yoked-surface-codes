...
def _observable_include_rounds(step: gen.StepOutline) -> dict[int, int]:
    rel_order_func = lambda _: gen.Order_Z
    ...
    step.build_rounds(
        ...
        cmp_layer=None,
        save_layer="test",
        edit_cur_obs={"x": set(), "z": set()},
        o2i={"x": 0, "z": 1},
    )

    result = {}
    round_index = 0
    for instruction in builder.circuit:
        if instruction.name == "OBSERVABLE_INCLUDE":
            observable_index = int(instruction.gate_args_copy()[0])
            result[observable_index] = round_index
        elif instruction.name == "SHIFT_COORDS":
            round_index += 1
    return result


def test_removing_step_transitions_preserves_observable_delta_timing():
    boundary = gen.ClosedCurve.from_cycle([
        0, "Z", 2, "X", 2 + 2j, "Z", 2j, "X",
    ])
    delta_x = boundary.with_basis("X")
    delta_z = boundary.with_basis("Z")
    step = gen.StepOutline(
        body=gen.PatchOutline([boundary]),
        obs_delta_body_start={"x": delta_x},
        obs_delta_body_end={"z": delta_z},
        rounds=2,
    )

    variants = [
        step,
        step.without_start_transition(),
        step.without_start_transition().without_start_transition(),
        step.without_end_transition(),
        step.without_end_transition().without_end_transition(),
    ]
    for variant in variants:
        assert variant.obs_delta_body_start == {"x": delta_x}
        assert variant.obs_delta_body_end == {"z": delta_z}
        assert _observable_include_rounds(variant) == {0: 0, 1: 1}


def test_step_outline_simple():
    rel_order_func = lambda m: gen.Order_Z if gen.checkerboard_basis(m) == 'Z' else gen.Order_ᴎ
    step = gen.StepOutline(
        ...
        body=gen.PatchOutline([gen.ClosedCurve.from_cycle([
            0, 'X', 2, 'Z', 2 + 2j, 'X', 2j, 'Z',
        ])]),
    ...
    builder = gen.Builder.for_qubits(step.body.to_patch(rel_order_func=rel_order_func).used_set)
    step.build_rounds(
        builder=builder,
        rel_order_func=rel_order_func,
        alternate_ordering_with_round_parity=False,
