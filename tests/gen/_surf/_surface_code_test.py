...
from gen._surf._surface_code import layer_single_shot, layer_transition, surface_code_patch
...
def test_layer_transition_notched_shift():
    ...
    p0 = PatchOutline([c0]).to_patch(rel_order_func=lambda m: Order_Z if checkerboard_basis(m) == 'Z' else Order_ᴎ)
    p2 = PatchOutline([c0.offset_by(2)]).to_patch(rel_order_func=lambda m: Order_Z if checkerboard_basis(m) == 'Z' else Order_ᴎ)
    p3 = PatchOutline([c0.offset_by(3)]).to_patch(rel_order_func=lambda m: Order_Z if checkerboard_basis(m) == 'Z' else Order_ᴎ)
...
def test_layer_transition_shrink():
    ...
    p_shrunk = PatchOutline([c_shrunk]).to_patch(rel_order_func=lambda m: Order_Z if checkerboard_basis(m) == 'Z' else Order_ᴎ)
    p_full = PatchOutline([c_full]).to_patch(rel_order_func=lambda m: Order_Z if checkerboard_basis(m) == 'Z' else Order_ᴎ)
...
def test_layer_transition_full_notch():
    ...
    p = PatchOutline([c]).to_patch(rel_order_func=lambda m: Order_Z if checkerboard_basis(m) == 'Z' else Order_ᴎ)
...
def test_layer_single_shot_default_data_obs_qubit_sets():
    patch = surface_code_patch(
        width=3,
        height=3,
        top_basis='Z',
    ...
    builder = Builder.for_qubits(patch.used_set)
    # Must not crash when data_obs_qubit_sets is left as its default (None).
    layer_single_shot(
        builder=builder,
        patch=patch,
        style='mpp',
    ...
    assert builder.circuit.num_measurements > 0
