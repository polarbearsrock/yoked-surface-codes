"""Hermetic tests for the remote paired Figure 8 campaign wrapper."""
...
WRAPPER = REPO_ROOT / "gcp" / "run_fig8_paired"
...
def _mini_repository(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    shutil.copy2(WRAPPER, repo / "gcp" / "run_fig8_paired")
...
""",
    ...
    benchmark = repo / "tools" / "benchmark_fig8_paired"
...
""",
    ...
    for executable in (activation, benchmark, plot, repo / "gcp/run_fig8_paired"):
...
def _run(
    ...
    *arguments: str,
    ...
    return subprocess.run(
        [str(repo / "gcp/run_fig8_paired"), *arguments],
        cwd=runtime,
        env=env,
        capture_output=True,
        text=True,
...
def _create(repo: Path, runtime: Path, log: Path, run_id: str = "science_001"):
    result = _run(
        repo,
        runtime,
        log,
    ...
    return runtime / "runs" / "fig8-paired" / run_id
...
def test_help_syntax_and_executable_mode() -> None:
    ...
    assert "create --run-id ID --p P --shots-per-cell N" in help_result.stdout
    assert help_result.stderr == ""
    assert syntax.returncode == 0
    assert os.access(WRAPPER, os.X_OK)


def test_create_uses_strict_campaign_path_and_exact_arguments(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stderr
    campaign = runtime / "runs/fig8-paired/science_001"
    assert campaign.is_dir()
    fields = _logged_fields(log)
    assert fields["tool"] == "benchmark"
    assert [fields[f"arg{k}"] for k in range(7)] == [
        "create",
        "--out",
        str(campaign),
        "--p",
...
def test_run_reasserts_exact_limits_and_forwards_only_32_processes(
    tmp_path: Path,
) -> None:
    repo, runtime, log = _mini_repository(tmp_path)
...
def test_run_rejects_activation_that_leaves_max_errors_set(tmp_path: Path) -> None:
    repo, runtime, log = _mini_repository(tmp_path)
    campaign = _create(repo, runtime, log)
    log.unlink()
...
def test_symlinked_campaign_or_parent_is_rejected(tmp_path: Path) -> None:
    ...
    external.mkdir()
    parent = runtime / "runs/fig8-paired"
    parent.mkdir()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("create", "--run-id", "x"), "create requires --p"),
        (
            ("create", "--run-id", "x", "--p", "0.1"),
            "create requires --shots-per-cell",
        ),
        (("status", "--run-id", "x", "--p", "0.1"), "accepts only --run-id"),
