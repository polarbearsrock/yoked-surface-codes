...
from yoked._squareberg_circuits import squareberg_magic_memory_circuit, \
    squareberg_phenomenological_circuit, make_squareberg_stabilizers
...
@pytest.mark.parametrize('num_patches', [True, 0, 4, 9, 15, 17, 32, 36, 100, 144])
def test_squareberg_num_patches_validation(num_patches: object):
    with pytest.raises(ValueError, match='power-of-two grid width'):
...
@pytest.mark.parametrize('w', [True, 0, 1, 2, 3, 6, 10, 12])
def test_squareberg_stabilizer_width_validation(w: object):
    with pytest.raises(ValueError, match='power of two at least 4'):
        make_squareberg_stabilizers(w)
