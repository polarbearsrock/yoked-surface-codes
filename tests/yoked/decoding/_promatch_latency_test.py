...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            LatencyProtocol(warmup_calls_per_variant=999),
            "warmup_calls_per_variant=1000",
        ),
        (LatencyProtocol(batch_sizes=(64,)), "batch size 1"),
        (LatencyProtocol(batch_sizes=(1, 2)), "only batch sizes"),
    ],
)
def test_scientific_mode_enforces_frozen_primary(
    protocol: LatencyProtocol, match: str
) -> None:
...
def test_restart_collects_direct_raw_calls_balanced_blocks_and_digests() -> None:
    ...
    counts = {
        name: sum(logged_name == name for logged_name, _ in factory.log)
        for name in {
            "u0_direct",
            "u0_wrap",
            "pu_window",
    ...
    assert all(
        value == "1"
        for value in record["provenance"]["runtime"][
...
def test_atomic_ledger_uses_tmpdir_and_refuses_accidental_overwrite(
    ...
    concurrent_destination = tmp_path / "results" / "concurrent.json"
    real_install = latency.install_bytes_atomic
    overwrite_arguments: list[bool] = []

    def install_after_competitor(path_arg, payload, **kwargs) -> None:
        raced_destination = Path(path_arg)
        raced_destination.write_bytes(b'{"owner":"competitor"}\n')
        overwrite_arguments.append(kwargs["overwrite"])
        real_install(path_arg, payload, **kwargs)

    monkeypatch.setattr(latency, "install_bytes_atomic", install_after_competitor)
    with pytest.raises(FileExistsError):
        write_restart_ledger_atomic(
    ...
    assert json.loads(concurrent_destination.read_text(encoding="utf-8")) == {
        "owner": "competitor"
    }
...
def test_orchestrator_rejects_unsafe_unrelated_and_immutable_outputs(
    tmp_path: Path,
) -> None:
    ...
    immutable = tmp_path / "promatch_l1_round1_v3" / "latency"
...
def test_orchestrator_persists_all_ledgers_and_resumes(
    ...
    # A caller may pre-create an empty output directory; it is still fresh.
    output.mkdir()
    ...
    for name in suite["restart_ledgers"]:
        ...
        assert (
            suite["restart_ledger_sha256"][name]
            == hashlib.sha256(canonical_json_bytes(ledger)).hexdigest()
        )

    completed_stats = {
        item.name: (item.stat().st_ino, item.stat().st_mtime_ns)
        for item in output.iterdir()
    }
    # A completed suite is a pure resume; no new executor is constructed or
    # previously verified artifact republished.
    ...
    # A recognized interrupted suite remains resumable: only the missing
    # restart is rerun and the already installed restart is left untouched.
    preserved = output / suite["restart_ledgers"][1]
    preserved_bytes = preserved.read_bytes()
    (output / "suite.json").unlink()
    (output / suite["restart_ledgers"][0]).unlink()
    resumed = run_latency_benchmark(
        _SyntheticFactory(),
        manifest=manifest,
        protocol=protocol,
        out_dir=output,
    ...
    assert resumed["schema"] == suite["schema"]
    assert preserved.read_bytes() == preserved_bytes
    assert all((output / name).is_file() for name in resumed["restart_ledgers"])
...
def test_latency_resume_preflight_rejects_without_mutation(
    ...
    if mutation == "missing-protocol":
        # Empty is a valid fresh root; a collection artifact without its
        # protocol is the malformed partial state this case exercises.
        ...
        restart_path.write_text("{}")
        (output / "suite.json").write_text('{"schema":"first","schema":"second"}')
...
def test_atomic_ledger_write_succeeds_without_tmpdir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.delenv("TMPDIR", raising=False)
    destination = tmp_path / "results" / "restart.json"
    write_restart_ledger_atomic(destination, {"schema": "test", "value": 3})
    ...
    assert {entry.name for entry in destination.parent.iterdir()} == {"restart.json"}
