...
def test_cell_id_rejects_dot_path_components(cell_id: str) -> None:
    ...
    with pytest.raises(ValueError, match="cell_id cannot be"):
        promatch_experiment._validate_cell_inputs(cell, require_hashes=False)
...
def test_freeze_hashes_the_input_template_before_normalization(
    ...
    monkeypatch.setattr(
        promatch_experiment, "normalize_protocol", stop_after_template_hash
    )
...
def test_documented_pilot_template_normalizes_to_disjoint_cell_schedules() -> None:
    path = REPO_ROOT / "docs" / "PROMATCH_PILOT_PROTOCOL.json"
    ...
    assert normalized["replay_policy"] == {
        ...
        "batch_ledger_selection": ("lowest_selection_sha256_within_batch_and_category"),
...
def test_documented_confirm_template_is_inspectable_but_not_freezable() -> None:
    path = REPO_ROOT / "docs" / "PROMATCH_FIRST_ROUND_PROTOCOL.json"
    ...
    assert normalized["performance_expected_shots_by_cell"] == {target_id: 1_000_000}
...
def test_documented_replay_policy_fails_closed() -> None:
    ...
    changed = copy.deepcopy(documented)
    changed["output_schema"]["bounded_replay_policy"]["invariant_violation_policy"] = (
...
def test_documented_output_contract_is_the_exact_executable_v3_contract() -> None:
    ...
    for name in (
        ...
        assert set(documented["output_schema"]["analysis_required_fields"]) == {
            "common_top_level",
            "pilot_additional_top_level",
            "cell",
        }
...
def test_frozen_normalized_protocol_revalidates_the_complete_output_contract() -> None:
    ...
    candidate["status"] = "FROZEN"
    candidate["frozen"] = True
    candidate["analysis_config"]["output_schema"]["artifact_schemas"]["summary"] = (
...
def test_target_validation_enforces_exact_performance_geometry_before_provenance() -> (
    None
):
    path = REPO_ROOT / "docs" / "PROMATCH_FIRST_ROUND_PROTOCOL.json"
...
def test_target_summary_is_explicitly_performance_only() -> None:
    summary = summarize_ledgers(
        ...
        experiment_id="0" * 64,
        phase="target",
        replay_policy=default_smoke_protocol(processes=1, shots=1)["replay_policy"],
    )
...
def test_summary_replay_cap_selects_global_lowest_hashes_across_batches() -> None:
    ...
    replay_policy["maximum_candidate_rows_per_category_per_batch_ledger"] = 2
    replay_policy["maximum_retained_rows_per_category_per_cell_summary"] = 2
...
def test_ledger_validation_rejects_noncanonical_replay_categories() -> None:
    ...
    regression_sample["pu_prediction_hex"] = "01" + "00" * (observable_width - 1)
    ...
    truncated_policy["maximum_candidate_rows_per_category_per_batch_ledger"] = 2
    truncated_policy["maximum_retained_rows_per_category_per_cell_summary"] = 2
...
def test_smoke_collection_resumes_from_complete_ledger(tmp_path) -> None:
    ...
    summary_before = (out / "summary.json").stat()
    ...
    summary_after = (out / "summary.json").stat()
    assert (summary_after.st_ino, summary_after.st_mtime_ns) == (
        summary_before.st_ino,
        summary_before.st_mtime_ns,
    )


def test_collection_rejects_unsafe_unrelated_and_immutable_output_roots(
    tmp_path: Path,
) -> None:
    ...
    assert {entry.name for entry in unrelated.iterdir()} == {"notes.txt"}
    ...
    immutable = tmp_path / "promatch_l1_round1_v3" / "new-output"
...
def test_collection_rejects_unrecognized_nested_batch_artifacts(tmp_path: Path) -> None:
    protocol = default_smoke_protocol(processes=1, shots=1)
    out = tmp_path / "paired"
    run_collection(protocol, phase="smoke", out=out, processes=1, scientific=False)
    extra = next((out / "batches").iterdir()) / "notes.txt"
    extra.write_text("not a ledger", encoding="utf-8")
    with pytest.raises(ValueError, match="unexpected or unsafe artifact"):
        run_collection(
            protocol, phase="smoke", out=out, processes=1, scientific=False
        )
    assert extra.read_text(encoding="utf-8") == "not a ledger"


def test_collection_preserves_valid_partial_resume(tmp_path: Path) -> None:
    protocol = default_smoke_protocol(processes=1, shots=4)
...
def test_atomic_json_write_succeeds_without_tmpdir(
    tmp_path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.delenv("TMPDIR", raising=False)
    path = tmp_path / "ledger" / "row.json"
    promatch_experiment._atomic_json_write(path, {"schema": "test", "value": 1})
    assert json.loads(path.read_text(encoding="utf-8")) == {
        "schema": "test",
        "value": 1,
    }
    # Staging happened next to the destination and left no residue behind.
    assert {entry.name for entry in path.parent.iterdir()} == {"row.json"}


def test_atomic_json_write_cannot_clobber_a_concurrent_creator(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = tmp_path / "ledger" / "row.json"
    real_install = promatch_experiment.install_bytes_atomic
    overwrite_arguments: list[bool] = []

    def install_after_competitor(path_arg, payload, **kwargs) -> None:
        destination = Path(path_arg)
        destination.parent.mkdir(parents=True, exist_ok=True)
        destination.write_bytes(b'{"owner":"competitor"}\n')
        overwrite_arguments.append(kwargs["overwrite"])
        real_install(path_arg, payload, **kwargs)

    monkeypatch.setattr(
        promatch_experiment,
        "install_bytes_atomic",
        install_after_competitor,
    )
    with pytest.raises(FileExistsError):
        promatch_experiment._atomic_json_write(path, {"owner": "collector"})
    assert overwrite_arguments == [False]
    assert json.loads(path.read_text(encoding="utf-8")) == {"owner": "competitor"}
