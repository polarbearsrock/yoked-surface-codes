...
def test_all_accept_stepper_matches_legacy_path_order_and_residual() -> None:
    ...
    assert stepped.durable_active == frozenset(np.flatnonzero(legacy.residual_syndrome))
...
def test_candidate_and_stage3_enumeration_telemetry_is_monotone() -> None:
    ...
    assert (
        stepper.last_candidate_enumeration_ns >= stepper.last_stage3_enumeration_ns >= 0
    )
    assert (
        stepper.total_candidate_enumeration_ns == stepper.last_candidate_enumeration_ns
    )
