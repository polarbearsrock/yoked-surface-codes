    ...
    def __call__(self, restart_index: int, batch_size: int) -> LatencyWorkload:
        ...
        provenance = {
            ...
            **self.identity.get("cell_hashes", {}),
            ...
            "corpus_shots_per_restart": self.identity["corpus_shots_per_restart"],
...
def _smoke_inputs(
    tmp_path: Path,
) -> tuple[Path, dict[str, Any], TinyLatencyAnalysisConfig]:
    ...
    suite = {
        ...
        "restart_ledger_sha256": {
            name: _canonical_file_sha256(output / name) for name in ledgers
        },
    }
...
def test_analyze_nonclaim_smoke_aggregates_frozen_shapes_and_gates(
    ...
    backend = result["batch_results"]["1"]["pairs"][BACKEND_RESIDUAL_VS_ORIGINAL]
    ...
    assert (
        result["batch_1_gates"]["end_to_end_software_latency_improvement_passed"]
        is True
    )
...
def _frozen_scientific_manifest() -> dict[str, Any]:
    document = REPO_ROOT / "docs" / "PROMATCH_FIRST_ROUND_PROTOCOL.json"
    ...
    manifest["analysis_config"]["selection"]["selected_cell_id"] = selected["cell_id"]
