...
def test_explicit_correlated_two_pass_matches_sinter_builtin() -> None:
    ...
    builtin_predictions = (
        sinter.BUILT_IN_DECODERS["pymatching-correlated"]
        .compile_decoder_for_dem(dem=dem)
        .decode_shots_bit_packed(bit_packed_detection_event_data=dets)
    )
...
def test_active_real_dem_preserves_global_gf2_algebra(yokes: int) -> None:
    ...
    for shot, residual_shot, frame, result in zip(unpacked, residual, frames, results):
        ...
        np.testing.assert_array_equal(
            residual_shot[list(protected)], shot[list(protected)]
        )
        assert not np.any(frame)


def test_production_adapter_calls_residual_matcher_once_and_retains_no_results() -> (
    None
):
