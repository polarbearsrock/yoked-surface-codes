...
def _compiled_graph(
    ...
    *,
    ...
    edges = edges_from_matcher(
        matcher, num_observables=num_observables, role=YokeDetector()
    )
...
def _three_way_graph(
    *,
    ...
    matcher.add_boundary_edge(0, weight=boundary_weights[0], fault_ids=set(left_faults))
...
class _ScriptedMatcher:
    def __init__(
        ...
        *,
        ...
        responses: dict[tuple[int, ...], tuple[list[int], float, list[tuple[int, int]]]]
        | None = None,
    ) -> None:
        self._base = base
        self._prediction = np.asarray(prediction, dtype=np.uint8)
        self._backend_weight = backend_weight
        self._returned_pairs = np.asarray(returned_pairs, dtype=np.int64).reshape(
            (-1, 2)
        )
...
def test_tau_weight_and_tau_k_have_their_distinct_scales() -> None:
    ...
    assert (
        classify_cost_excess(cost_excess=0.95, tau_k=0.95)
        is CostClassification.COMPATIBLE
    )
    assert (
        classify_cost_excess(cost_excess=0.951, tau_k=0.95)
        is CostClassification.POSITIVE_EXCESS
    )
    assert (
        classify_cost_excess(cost_excess=-0.951, tau_k=0.95)
        is CostClassification.ACCOUNTING_ANOMALY
    )
...
def _support_boundary(
    graph: CompiledPromatchGraph, support_mask: int
) -> tuple[int, ...]:
...
def test_exact_small_graph_support_containment_theorem(
    ...
    for syndrome_boundary, (minimum_weight, minimum_supports) in minima.items():
        ...
        for candidate in range(1 << len(graph.edges)):
            candidate_ids = tuple(
                edge.edge_id for edge in graph.edges if candidate & (1 << edge.edge_id)
            )
