...
from yoked.decoding.oracle.policy_analysis import canonical_json_bytes
from yoked.decoding.oracle import policy_casebook as casebook
from yoked.decoding.oracle import policy_experiment
...
def _fixture_root(
    ...
    *,
    ...
    analysis = {
        ...
        "tables": {
            "fatal_gates": [
                {
                    "gate": k,
                    "status": "passed-ledger-recomputed",
                    "evidence": f"gate {k}",
                }
                for k in range(1, 19)
            ]
    ...
    monkeypatch.setattr(
        casebook, "validate_policy_protocol", lambda *args, **kwargs: None
    )
    ...
    graph = SimpleNamespace(
        ...
        layout=SimpleNamespace(
            ...
            roles=(SimpleNamespace(patch_id=0, check_basis="X", time=0, window_id=0),)
            * 8,
        ),
    )
    ...
    def expand(graph_arg, syndrome, **kwargs):
        ...
        rows = [
            ...
            },
            {
                ...
                "exhaustion_kind": "proposal",
                "counterfactual_wall_ns": 888,
            },
        ]
...
def test_authenticated_expansion_is_detector_only_atomic_and_idempotent(
    ...
    assert snapshot["edges"] == [
        {
            ...
            "membership": [
                "candidate-P",
                "support-difference-X",
                "candidate-relevant-X",
                "support-cancellation-P-intersection-R",
            ],
            "source": 0,
            "support_difference_ranks": [2],
            "target": 1,
            "candidate_relevant_ranks": [2],
            "disconnected_ranks": [],
        ...
        },
        {
            ...
            "source": 2,
            "support_difference_ranks": [1],
            "target": 3,
            "candidate_relevant_ranks": [],
            "disconnected_ranks": [1],
    ...
    assert (
        casebook.expand_authenticated_policy_casebook(root, config=config) == manifest
    )
...
def test_casebook_normalizes_in_domain_across_candidate_relevant_components() -> None:
    ...
    with pytest.raises(
        casebook.PolicyCasebookError, match="candidate-relevant component"
    ):
...
def test_expansion_tamper_and_complete_are_fail_closed(
    ...
    with pytest.raises(
        casebook.PolicyCasebookError, match="expansion artifact differs"
    ):
...
def test_ground_truth_firewall_rejects_the_full_union_at_both_layers() -> None:
    union = policy_experiment.GROUND_TRUTH_FORBIDDEN_KEYS
    assert policy_experiment.CASEBOOK_EXTRA_GROUND_TRUTH_KEYS <= union
    assert {"arm_failure", "failed", "logical_failure", "posthoc"} <= union
    assert {"actual_observables", "arm_failures", "posthoc_ground_truth"} <= union
    for key in sorted(union):
        payload = {"nested": [{key: 1}]}
        with pytest.raises(ValueError, match="ground-truth-like"):
            policy_experiment.forbid_ground_truth_keys(payload)
        with pytest.raises(casebook.PolicyCasebookError, match="ground-truth-like"):
            casebook._reject_ground_truth_keys(payload)


def test_importing_casebook_does_not_import_matplotlib_pyplot() -> None:
    # pymatching itself imports matplotlib.cbook at module scope, so bare
    # "matplotlib" cannot be kept out of sys.modules; the casebook's own
    # contribution is pyplot plus the backend/MPLCONFIGDIR side effects, and
    # those must stay confined to the diagram-rendering path.
    root = REPO_ROOT
    ...
    subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys; import yoked.decoding.oracle.policy_casebook; "
            "assert 'matplotlib.pyplot' not in sys.modules, "
            "'matplotlib.pyplot must stay out of the casebook import path'",
        ],
        check=True,
        env=environment,
    )
