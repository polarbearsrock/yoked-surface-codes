...
from tests.yoked.decoding.oracle.conftest import edges_from_matcher
...
from yoked.decoding.oracle.policy_audit import (
    ARM_IDS,
...
from yoked.decoding.oracle.policy_experiment import _audit_policy_shot
...
def _unsafe_graph() -> CompiledPromatchGraph:
    ...
    edges = edges_from_matcher(
        matcher, num_observables=1, role=L1BodyDetector(0, "X", 0, 0)
    )
    local = None
    for edge in edges:
...
def _evaluation(
    *, base=(), candidate=(), residual=(), frame_compatible=True, accepted=False
    ...
    return OracleEvaluation(
        policy="frame",
        accepted=accepted,
        cost_compatible=accepted,
        frame_compatible=frame_compatible,
        cost_classification=(
            CostClassification.COMPATIBLE
            if accepted
            else CostClassification.POSITIVE_EXCESS
        ),
        cost_excess=0.0 if accepted else 1.0,
        tau_k=1e-6,
        composite_weight=1.0,
        accumulated_frame=b"\0",
        base_prediction=b"\0",
        residual_prediction=b"\0",
        base_frame=b"\0",
        candidate_composite_frame=b"\0" if frame_compatible else b"\1",
        base_support_edge_ids=tuple(base),
        residual_support_edge_ids=tuple(residual),
        base_support_weight=0.0,
        residual_support_weight=0.0,
        base_backend_weight=0.0,
        residual_backend_weight=0.0,
        base_tau_weight=1e-6,
        residual_tau_weight=1e-6,
        candidate_edge_ids=tuple(candidate),
        candidate_boundary_detector_ids=(0, 1),
        candidate_observable_frame=b"\0",
...
def test_difference_components_retain_remote_x_and_independent_cancellation() -> None:
    ...
    local = next(
        edge.edge_id for edge in graph.edges if edge.target == 1 and edge.source == 0
    )
    remote = next(
        edge.edge_id for edge in graph.edges if edge.source == 2 and edge.target is None
    )
    ...
    cancellation = [
        row for row in components if row["certificate_kind"] == "support-cancellation"
    ]
...
def test_difference_components_accept_x_empty_cancellation_and_reject_unexplained() -> (
    None
):
    graph = _unsafe_graph()
    domain = next(iter(graph.domain_graphs))
...
def _rollback_accounting_graph() -> CompiledPromatchGraph:
    ...
    local = [edge for edge in edges if edge.target is not None]
    domain = L1FullHistoryDomain(0, "X")
    ...
    domain_graph = DomainGraph(
        ...
        {k: tuple(v) for k, v in neighbors.items()},
        (),
        {k: () for k in range(14)},
    )
    return CompiledPromatchGraph(
...
def test_policy_audit_is_ground_truth_free_deterministic_and_adapter_compatible(
    graph,
) -> None:
...
def test_context_views_are_sorted_and_separate_on_a_nontrivial_shot(graph) -> None:
    ...
    domain = next(
        d
        for d in sorted(graph.domain_graphs)
        if len(graph.domain_graphs[d].detector_ids) > 10
    )
    ...
    for row in result["proposals"] + result["counterfactuals"]:
        assert row["matched_partner_labels"] == sorted(
            set(row["matched_partner_labels"])
        )
        ...
        assert row["omitted_context_labels"] == sorted(
            set(row["omitted_context_labels"])
        )
        ...
        assert row["degeneracy_diagnostics"] == sorted(
            set(row["degeneracy_diagnostics"])
        )
        ...
        assert row["support_cancellation_edge_ids"] == sorted(
            set(row["candidate_support_edge_ids"]).intersection(
                row["residual_support_edge_ids"]
            )
        )
...
def test_selected_support_does_not_label_an_absent_proposal_endpoint() -> None:
    ...
    boundary_edge = next(
        ...
        if edge.target is None and edge.source not in {0, 1}
    )
    assert (
        _support_component_labels(graph, [boundary_edge.edge_id], (0, 1), domain) == []
    )


def test_transaction_and_partial_workload_accounting_diverge_after_rollback() -> None:
    result = audit_policy_shot(
        _rollback_accounting_graph(),
        np.ones(14, dtype=np.uint8),
        tolerance=OracleTolerance(),
    )
    ...
    assert (
        partial["durable_events_removed"] == partial["provisional_events_removed"] > 0
    )
...
def test_casebook_expansion_continues_after_first_safe_to_true_exhaustion() -> None:
    ...
    assert [row["operational_veto_chain_rank"] for row in rows] == list(
        range(1, len(rows) + 1)
    )
...
def test_matched_active_pair_validation_rejects_malformed_backend(
    pairs, message
) -> None:
    class Matcher:
    ...
    with pytest.raises(AssertionError, match=message):
        _matched_partner_labels(
            fake, np.asarray([1, 0, 0], dtype=np.uint8), (0, 1), domain
        )
