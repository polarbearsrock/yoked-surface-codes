...
def test_run_until_done_propagates_interrupt_after_stopping_workers():
    manager = object.__new__(CollectionManager)
    manager.task_states = {'pending': object()}
    manager.process_message = lambda: (_ for _ in ()).throw(KeyboardInterrupt())
    stopped = []
    manager.hard_stop = lambda: stopped.append(True)
...
"""
    ...
    assert_drain_queue(manager.worker_states[0].input_queue, [
        (
            'change_job',
            (t0, 0),
        ),
    ...
    assert_drain_queue(manager.worker_states[1].input_queue, [
    ...
    assert_drain_queue(manager.worker_states[2].input_queue, [
    ...
    assert log.pop() is None
    assert not log
    put_wait_not_empty(manager.shared_worker_output_queue, (
        'flushed_results',
        2,
