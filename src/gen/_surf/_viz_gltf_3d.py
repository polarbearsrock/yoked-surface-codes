...
class ColoredTriangleData:
    def __init__(self, *, rgba: Tuple[float, float, float, float], triangle_list: np.ndarray):
        assert len(triangle_list.shape) == 2 and triangle_list.shape[1] == 3 and triangle_list.dtype == np.float32
        assert len(rgba) == 4
    ...
    @staticmethod
    def square(*, rgba: Tuple[float, float, float, float], origin: Iterable[float], d1: Iterable[float], d2: Iterable[float]) -> 'ColoredTriangleData':
    ...
    def fused(data: Iterable['ColoredTriangleData']) -> List['ColoredTriangleData']:
        ...
        for rgba, group in groups.items():
            if len(group) == 1:
                ...
                result.append(ColoredTriangleData(
                    rgba=rgba,
...
class ColoredLineData:
    def __init__(self, *, rgba: Tuple[float, float, float, float], edge_list: np.ndarray):
        assert len(edge_list.shape) == 2 and edge_list.shape[1] == 3 and edge_list.dtype == np.float32
        assert len(rgba) == 4
...
def gltf_model_from_colored_triangle_data(
    ...
    coords_tri = np.array([]) if not colored_triangle_data else np.concatenate([data.triangle_list for data in colored_triangle_data], axis=0)
    coords_edg = np.array([]) if not colored_line_data else np.concatenate([data.edge_list for data in colored_line_data], axis=0)
...
<body>
  <a download="model.gltf" id="stim-3d-viewer-download-link" href="''' + model_data_uri + r'''">Download 3D Model as .GLTF File</a>
