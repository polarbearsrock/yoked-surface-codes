...
def test_tight_boundary():
    ...
    p = b.to_patch(rel_order_func=lambda m: Order_Z if checkerboard_basis(m) == 'Z' else Order_ᴎ)
...
def test_pitchfork_boundary():
    b = PatchOutline([
        ClosedCurve(points=[0, 3, 3 + 10j, 2 + 10j, 2 + 6j, 1 + 6j, 1 + 10j, 10j],
              bases='XXXXZXXX'),
    ])
    p = b.to_patch(rel_order_func=lambda m: Order_Z if checkerboard_basis(m) == 'Z' else Order_ᴎ)
...
def test_fused_t():
    ...
    assert len([p for p in int_points_inside_polygon(b.region_curves[0].points, include_boundary=False) if p.real == 0]) == 3
...
def test_fused_inner():
    b1 = ClosedCurve.from_cycle([
        'X', 0+16j, 6+16j, 8+16j, 14+16j, 16+16j, 22+16j,
        22+18j, 22+20j, 22+22j, 22+24j, 22+26j, 22+28j, 22+30j,
        16+30j, 16+28j, 16+26j,
        14+26j, 8+26j,
        'Z', 8+24j, 'X', 14+24j, 16+24j,
        16+22j, 16+20j, 16+18j,
        14+18j, 8+18j, 6+18j,
        6+20j, 8+20j, 14+20j,
        'Z', 14+22j, 'X', 8+22j, 6+22j,
        6+24j, 6+26j, 6+28j,
        8+28j, 14+28j,
        'Z', 14+30j, 'X', 8+30j, 6+30j,
