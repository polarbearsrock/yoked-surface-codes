...
from gen._core import NoiseModel, Patch
from gen._flows import Chunk, ChunkLoop, compile_chunks_into_circuit
...
class CircuitFactory:
    handles_noise: bool
    custom_keys: Dict[str, type]
    make_circuit: Callable[[CircuitBuildParams], Union[stim.Circuit, List[Union[Chunk, ChunkLoop]]]]
...
def _generate_single_circuit(
    ...
    if not isinstance(construction, CircuitFactory):
        construction = CircuitFactory(handles_noise=False, custom_keys={}, make_circuit=construction)
    chunks = construction.make_circuit(params)
    if construction.handles_noise:
        assert isinstance(chunks, stim.Circuit)
        if debug_out_dir is not None:
            write_file(debug_out_dir / "noisy_circuit.html", stim_circuit_html_viewer(chunks))
            write_file(debug_out_dir / "noisy_circuit.stim", chunks)
            write_file(debug_out_dir / "noisy_circuit_dets.svg", chunks.diagram("time+detector-slice-svg"))
        return chunks
...
def generate_noisy_circuit_from_chunks(
        *,
        chunks: Union[stim.Circuit, List[Union[Chunk, ChunkLoop]]],
        noise: Optional[NoiseModel],
    ...
    if isinstance(chunks, stim.Circuit):
        chunks = [Chunk(
            circuit=chunks,
            q2i={r + 1j*i: k for k, (r, i) in chunks.get_final_qubit_coordinates().items()},
            flows=[],
        )]
    if debug_out_dir is not None:
        debug_out_dir = pathlib.Path(debug_out_dir)
    ...
    if convert_to_cz:
        body = transpile_to_z_basis_interaction_circuit(body, is_entire_circuit=len(magic_head) == len(magic_tail) == 0)
        if debug_out_dir is not None:
