    ...
    *,
    ...
    for raw_fault_id in values:
        ...
        if fault_id < 0 or fault_id >= num_observables:
            raise OracleGraphError(
                f"matching observable ID {fault_id} outside "
                f"[0, {num_observables})"
            )
        if fault_id in seen:
...
class FullGraphOracle:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(graph, CompiledPromatchGraph):
            raise TypeError(
                "graph must be a CompiledPromatchGraph; "
                f"got {type(graph)!r}"
            )
    ...
    def _validate_graph(
        ...
        for edge in graph.edges:
            ...
            if unused_bits and edge.observable_mask and (
                edge.observable_mask[-1] >> (8 - unused_bits)
            ):
                raise OracleGraphError(
        ...
        for raw_source, raw_target, data in matcher.edges():
            ...
            if pair in matcher_by_pair:
                raise OracleGraphError(
                    f"ambiguous matcher endpoint pair {pair!r}"
                )
            try:
    ...
    def decode_state(
        ...
        *,
        ...
        for raw_source, raw_target in returned_pairs:
            ...
            support_mask = _xor_masks(
                support_mask, edge.observable_mask
            )
        edge_ids.sort()
    ...
    def evaluate(
        ...
        *,
        ...
        residual_weights = [
            self._edge_by_id[edge_id].weight
            for edge_id in residual.support_edge_ids
        ]
        ...
        classification = classify_cost_excess(
            cost_excess=cost_excess, tau_k=tau_k
        )
        if classification is CostClassification.ACCOUNTING_ANOMALY:
            raise OracleAccountingError(cost_excess=cost_excess, tau_k=tau_k)

        base_frame = _xor_masks(accumulated_mask, base.prediction)
        candidate_accumulated = _xor_masks(
            accumulated_mask, candidate_observable_frame
        )
