...
def _load_tool():
    path = ROOT / "tools" / "diagnose_promatch_l1"
...
def _config(input_dir: Path, *, experiment_id: str, protocol_hash: str, summary_hash: str):
    config = {
...
def test_oracle_output_refuses_input_descendants_and_nonempty(tmp_path: Path) -> None:
    ...
    assert TOOL._ensure_oracle_output_target(corpus, tmp_path / "new") == (tmp_path / "new").resolve()
...
def test_strict_loader_deduplicates_physical_shot_and_keeps_selection_labels(
    ...
    summary = {
        ...
        "cells": [{
            "cell_id": "cell-a", "batches": 1, "shots": 1,
            "paired_contingency": {}, "telemetry": {}, "replay_samples": samples,
        }],
    }
    ...
    monkeypatch.setattr(
        TOOL, "validate_experiment_protocol", lambda manifest, **kwargs: manifest["experiment_id"]
    )
...
def test_frozen_config_rejects_source_hash_drift(tmp_path: Path) -> None:
    config = _config(
        tmp_path,
        experiment_id="2" * 64,
        protocol_hash="3" * 64,
    ...
    semantic = dict(config)
    del semantic["experiment_id"]
    config["experiment_id"] = hashlib.sha256(TOOL._canonical_json_bytes(semantic)).hexdigest()
...
def test_clean_implementation_allows_only_the_post_freeze_config_commit(
    ...
    implementation = "1" * 40
    head = "2" * 40
    config_path = ROOT / "docs" / "ORACLE_FIXTURE.json"
    ...
    monkeypatch.setattr(
        TOOL, "_git_output",
        lambda *args: (
            "" if args[0] == "status" else head if args[0] == "rev-parse"
...
def test_run_oracle_replay_shot_exercises_all_arms_without_observables_in_oracle(
    ...
    shots = [{
        ...
        "stim_seed": 0,
        "selection_labels": [],
        "detection_events_hex": bytes(np.packbits(syndrome, bitorder="little")).hex(),
    ...
    assert all(
        summary["pooled_arms"][arm]["logical_failures"] == 0
        for arm in TOOL.ORACLE_ARMS
    )
...
def _numeric_characterization_fixture(
    *, residual: float, base_backend: float = 1.0
):
    role = YokeDetector()
    edges = (
    ...
    values = {
        ...
        "base_tau_weight": 1e-9 + 1e-6 * max(
            1.0, abs(base), abs(base_backend)
        ),
        "residual_tau_weight": 1e-9 + 1e-6 * max(
            1.0, abs(residual), abs(residual)
        ),
    }
    evaluation = {
