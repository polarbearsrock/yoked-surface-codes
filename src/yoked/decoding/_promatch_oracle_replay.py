...
def _xor_masks(left: bytes, right: bytes) -> bytes:
    if len(left) != len(right):
        raise ValueError("observable masks have inconsistent lengths")
...
def run_oracle_trajectory(
    ...
    *,
    ...
    if transaction_policy not in ("tx", "partial"):
        raise ValueError(
            f"unsupported transaction policy {transaction_policy!r}"
        )
    if oracle is None:
        oracle = FullGraphOracle(graph, tolerance=tolerance)
    elif oracle.graph is not graph:
    ...
    for domain in sorted(graph.domain_graphs):
        ...
        if overlap:
            raise ValueError(
                "detectors belong to multiple predecode domains: "
                f"{sorted(overlap)}"
            )
        ...
        while (proposal := stepper.next_proposal()) is not None:
            ...
            if evaluation.accepted:
                ...
                residual = apply_detector_boundary(
                    residual, proposal.detector_boundary
                )
...
def run_phase_a_replay(
    ...
    *,
    ...
    cost_tx = run_oracle_trajectory(
        **common, policy="cost", transaction_policy="tx"
    )
    frame_tx = run_oracle_trajectory(
        **common, policy="frame", transaction_policy="tx"
    )
