...
Expansion recomputes the complete candidate slate only for states selected by
the offline analysis. Installation is atomic and verification regenerates the
expected payloads before finalization marks an audit complete.
"""
...
from yoked.decoding.oracle.policy_audit import expand_policy_casebook_state
...
# This layer's historically extra ground-truth spellings; they live in
# policy_experiment as an explicit additive frozen set folded into the shared
# GROUND_TRUTH_FORBIDDEN_KEYS union that both firewall layers enforce.
_CASEBOOK_EXTRA_GROUND_TRUTH_KEYS = CASEBOOK_EXTRA_GROUND_TRUTH_KEYS
_SUPPORT_DIFFERENCE_REPRESENTATION = "promatch-support-difference-v2"
_SUPPORT_COMPONENT_FIELDS = SUPPORT_COMPONENT_FIELDS_V2


class PolicyCasebookError(ValueError):
    """A casebook input or installed artifact violates its frozen contract."""
...
def _state_id(row: Mapping[str, Any]) -> str:
    ...
    identity = [
        row.get("cell_id"),
        row.get("worker_id"),
        row.get("global_shot_id"),
        [row.get("patch_id"), row.get("basis"), row.get("window_id")],
        state,
    ]
    if not isinstance(state, str) or not state:
        raise PolicyCasebookError(
            "rank-one counterfactual lacks original state identity"
        )
...
def _reject_ground_truth_keys(value: Any, *, path: str = "root") -> None:
    forbid_ground_truth_keys(value, path=path, error=PolicyCasebookError)
        ...
        for key, item in value.items():
            if isinstance(item, float):
                ...
                if companion != item.hex():
                    raise PolicyCasebookError(
                        f"float lacks exact hex companion at {path}.{key}"
                    )
...
def _canonical_edge_ids(value: Any, *, field: str) -> list[int]:
    if not isinstance(value, list) or any(
        ...
        raise PolicyCasebookError(
            f"{field} must be an array of nonnegative integer edge IDs"
        )
...
def _support_evidence(row: Mapping[str, Any], *, graph: Any) -> dict[str, Any]:
    ...
    if not set(cancellation).issubset(candidate):
        raise PolicyCasebookError(
            "P_intersection_R_edge_ids must be a subset of candidate P"
        )
    ...
    if not isinstance(disconnected_flag, bool):
        raise PolicyCasebookError(
            "disconnected_support_reconfiguration must be boolean"
        )
    ...
    for index, raw_component in enumerate(components):
        if (
            ...
            or set(raw_component) != _SUPPORT_COMPONENT_FIELDS
        ):
            raise PolicyCasebookError(
        ...
        if kind == "real-x-component":
            if seen_cancellation:
                raise PolicyCasebookError(
                    "real X components must precede support-cancellation"
                )
            ...
            if previous_real_key is not None and real_key <= previous_real_key:
                raise PolicyCasebookError(
                    "real X components must be in canonical edge order"
                )
            ...
            if overlap:
                raise PolicyCasebookError(
                    "real X components contain duplicate edge membership"
                )
            ...
            if seen_cancellation:
                raise PolicyCasebookError(
                    "support-cancellation certificate must be unique"
                )
...
def _support_graph_snapshot(
    ...
    *,
    ...
    active = {
        int(detector)
        for detector in rows[0].get("local_active_state_fingerprint", [])
        if int(detector) >= 0
    }
    ...
    for edge_id in edge_ids:
        try:
            ...
            raise PolicyCasebookError(
                f"support snapshot references unknown edge {edge_id}"
            ) from ex
        ...
        detectors.append(
            {
                ...
                "active_local": detector_id in active,
...
def _import_pyplot():
    """Imports matplotlib only on the rendering path.

    Callers that authenticate, verify, or finalize a casebook must be able to
    import this module without pulling in matplotlib, so the MPLCONFIGDIR
    redirection and backend selection happen here rather than at module scope.
    """
...
def _render_state_diagram(state: Mapping[str, Any], path: Path) -> None:
    ...
    remote_roles = sorted(
        {
            label
            for item in state["all_veto_timeline"]
    ...
    for rank, value, color, marker, stage in zip(
        ranks, excess, colors, markers, stages
    ):
        ...
        ax.annotate(
            f"P={path_count}, X={difference_count}",
            (rank, value),
            xytext=(0, -13),
            textcoords="offset points",
            ha="center",
    ...
    for detector in snapshot["detectors"]:
        ...
        graph_ax.scatter(
            [x],
            [y],
            s=48 if detector["active_local"] else 25,
    ...
    graph_ax.plot(
        [], [], color="#888888", linestyle="--", linewidth=1.7, label="disconnected X"
    )
    graph_ax.scatter([], [], color="#222222", s=48, label="active local detector")
    graph_ax.set(
        xlabel="detector coordinate x",
        ylabel="detector coordinate y",
...
def _authenticate_inputs(
    ...
    *,
    ...
    if corpus.config != supplied:
        raise PolicyCasebookError(
            "supplied config differs from installed authenticated config"
        )
    ...
    if supplied.get("frozen") is True:
        ...
        expected = (
            repository_root / str(supplied.get("protocol_relative_path"))
        ).resolve()
        ...
        if (
            ...
            raise PolicyCasebookError(
                "frozen protocol path is not the exact repository docs path"
            )
    ...
    if selection != analysis.get("casebook_selection"):
        raise PolicyCasebookError(
            "installed selection differs from recomputed analysis"
        )
...
def _selected_rank_one_rows(
    corpus: Any, selection: Mapping[str, Any]
    ...
    for state_id, row in selected.items():
        ...
        if row["state_id"] != state_id or not isinstance(
            row["selection_reasons"], list
        ):
    ...
    for row in corpus.rows["counterfactuals"]:
        ...
        if state_id in selected:
            ...
            if row.get("original_proposal_sha256") != selected[state_id].get(
                "original_proposal_sha256"
            ):
                raise PolicyCasebookError(
                    "selection/original proposal identity mismatch"
                )
            found[state_id] = dict(row)
    if set(found) != set(selected):
        raise PolicyCasebookError(
            "not every selected state exists in retained detector ledgers"
        )
    return found
...
def _expansion_payloads(
    *, corpus: Any, selection: Mapping[str, Any], config: Mapping[str, Any]
    ...
    prepared = prepare_cell(
        ...
        verify_hashes=bool(config.get("frozen")),
    )
    ...
    for state_id in sorted(rank_one):
        ...
        syndrome = np.unpackbits(
            np.frombuffer(detector_bytes, dtype=np.uint8),
            bitorder="little",
        ...
        for row in expanded:
            if (
                ...
                raise PolicyCasebookError(
                    "core expansion returned the wrong selected-state identity"
                )
        identity = {
            name: original[name]
            for name in (
        ...
        if rows[0]["proposal_sha256"] != original["original_proposal_sha256"]:
            raise PolicyCasebookError(
                "exhaustive rank one differs from selected original"
            )
        if (
            rows[-1].get("terminal_action") != "exhaustive-true-exhaustion"
            or rows[-1].get("exhaustion_kind") != "proposal"
        ):
        ...
        if any(
            ...
            raise PolicyCasebookError(
                "casebook row schema or graph/layout identity is incomplete"
            )
        ...
        first_safe = next(
            (
                row["operational_veto_chain_rank"]
                for row in rows
                if row["oracle_policy_accepts"]
            ),
            None,
        )
        contexts = sorted(
        ...
        visibility = sorted(
            {
                label
                for row in rows
                for label in row.get("feature_visibility", {}).values()
            }
        )
            ...
            "all_veto_timeline": [
                {
                    ...
                    "support_difference_edge_ids": evidence[
                        "support_difference_edge_ids"
                    ],
                    ...
                    "remote_support_role_labels": evidence[
                        "remote_support_role_labels"
                    ],
                }
        ...
        state_payloads[f"states/{state_id}.json"] = (
            canonical_json_bytes(state_object) + b"\n"
        )
        all_rows.extend(rows)
    exhaustive = b"".join(canonical_json_bytes(row) + b"\n" for row in all_rows)
    return all_rows, {
        "exhaustive.jsonl.gz": _deterministic_gzip(exhaustive),
        **state_payloads,
    }
...
def _verify_expansion_files(
    ...
    *,
    ...
    expected_fields = {
        ...
        "diagram_images",
        "diagram_images_scientifically_digested",
        "timing_fields_removed_from_scientific_rows",
    }
    if (
        set(manifest) != expected_fields
        or manifest.get("schema") != EXPANSION_MANIFEST_SCHEMA
    ):
    ...
    if (
        ...
        raise PolicyCasebookError(
            "expansion manifest identity differs from authenticated inputs"
        )
    expected_hashes = {
        name: _sha(data) for name, data in sorted(expected_payloads.items())
    }
    if manifest.get("scientific_file_sha256") != expected_hashes:
        raise PolicyCasebookError(
            "expansion scientific hashes differ from recomputation"
        )
    ...
    if (
        not isinstance(diagrams, list)
        or manifest.get("diagram_images_scientifically_digested") is not False
    ):
    ...
    if ready != expected_ready:
        raise PolicyCasebookError(
            "EXPANSION_READY does not authenticate expansion manifest"
        )
    return manifest
...
def expand_authenticated_policy_casebook(
    ...
    *,
    ...
    rows, payloads = _expansion_payloads(
        corpus=corpus, selection=selection, config=config
    )
    ...
    # The whole expansion DIRECTORY is installed with one os.replace below,
    # which has no cross-device fallback, so the same-filesystem precondition
    # must stay even though single-file installs no longer need it.
    if os.stat(scratch).st_dev != os.stat(root).st_dev:
        raise RuntimeError("TMPDIR and corpus must share a filesystem")
    temporary = Path(
        tempfile.mkdtemp(prefix="promatch-casebook-expansion-", dir=scratch)
    )
    installed = False
    try:
        ...
        manifest = {
            ...
            "selection_sha256": _sha(canonical_json_bytes(selection) + b"\n"),
            "selected_states": len(by_state),
            "exhaustive_rows": len(rows),
            "scientific_file_sha256": {
        ...
        install_bytes_atomic(
            ...
            prefix="promatch-expansion-ready-",
        )
        return _verify_expansion_files(
            root,
            expected_payloads=payloads,
...
def verify_policy_casebook_expansion(
    ...
    *,
    ...
    """Recomputes and verifies every installed detector-only casebook artifact."""
...
def finalize_policy_audit(
    ...
    *,
    ...
    root = Path(root).resolve()
    corpus, analysis, _ = _authenticate_inputs(
        root, config=config, protocol_path=protocol_path
    )
    ...
    complete = {
        ...
        "analysis_manifest_sha256": _sha(
            (root / "analysis" / "manifest.json").read_bytes()
        ),
        "expansion_manifest_sha256": _sha(
            (root / "casebook" / "expansion" / "manifest.json").read_bytes()
        ),
        "analysis_sha256": analysis["analysis_sha256"],
    ...
    if target.exists():
        if target.is_symlink() or target.read_bytes() != data:
            raise PolicyCasebookError(
                "existing COMPLETE differs from authenticated finalization"
            )
        return complete
    # install_bytes_atomic stages in TMPDIR when set and restages next to the
    # destination on a cross-device rename, so the previous TMPDIR/same-
    # filesystem preconditions are no longer needed for this single file.
    install_bytes_atomic(target, data, prefix="promatch-complete-")
