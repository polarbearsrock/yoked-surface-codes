...
from yoked.decoding._promatch_policy_experiment import (
    ANALYSIS_PLOT_NAMES,
...
class _FakeMatcher:
    def decode_batch(self, dets, *, bit_packed_shots: bool, bit_packed_predictions: bool):
...
def _rewrite_first_ledger_row(payloads: dict[str, bytes], name: str, mutate) -> None:
    ...
    rows = [json.loads(line) for line in gzip.decompress(payloads[filename]).splitlines()]
    mutate(rows[0])
    raw = b"".join(
...
def _fake_completed_probe(
    root: Path, config: dict, *, commit: str = "a" * 40,
    gates_passed: bool = True, observed_pids: int = 32,
    ...
    tables["fatal_gates"] = [
        {"gate": gate, "status": "passed-ledger-recomputed"}
...
def test_seed_derivation_is_deterministic_and_worker_separated() -> None:
    ...
    assert derive_policy_worker_seed(**kwargs, worker_id=0) == derive_policy_worker_seed(
        **kwargs, worker_id=0
    )
    assert derive_policy_worker_seed(**kwargs, worker_id=0) != derive_policy_worker_seed(
        **kwargs, worker_id=1
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda value: value.update(unvalidated_semantics=True), "fields must be exactly"),
    ],
)
...
def test_fresh_shard_is_verified_before_atomic_install(
    ...
    with pytest.raises(ValueError, match="prediction arm set"):
        install_worker_shard(
            tmp_path / "invalid", shard=shard, payloads=payloads,
            config=config, mode="smoke", spec=spec,
        )
...
def test_scientific_ledgers_are_bit_exact_while_timing_is_excluded() -> None:
    ...
    assert timing["core_timing_by_ledger"]["proposals"][0]["timing"] != (
    ...
    assert set(timing["serialization_by_artifact"]) == {
        "shots.jsonl.gz", "proposals.jsonl.gz", "counterfactuals.jsonl.gz",
        "domains.jsonl.gz",
    }
...
def test_probe_attestation_requires_analysis_and_exact_materialized_draft(
    ...
    monkeypatch.setattr(
        "yoked.decoding._promatch_policy_experiment.verify_worker_shard",
...
def test_probe_attestation_requires_32_processes_and_rendered_plots(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
    observed_pids: int, plots_rendered: bool, message: str,
) -> None:
    config = _runtime_config()
    commit = "a" * 40
    shards = _fake_completed_probe(
        tmp_path / "probe", config, commit=commit,
        observed_pids=observed_pids, plots_rendered=plots_rendered,
    )
    monkeypatch.setattr(
        "yoked.decoding._promatch_policy_experiment.verify_worker_shard",
        lambda path, *, config, mode, spec: shards[spec.worker_id],
