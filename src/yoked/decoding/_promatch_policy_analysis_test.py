...
def _shot(shot: int, *, u0: str, shadow: str, u0_failed: bool, shadow_failed: bool) -> dict:
    predictions = {
        ARM_IDS[0]: u0,
        ARM_IDS[1]: shadow,
        ARM_IDS[2]: u0,
...
def _counterfactual_chain(original: dict, state_digit: str, *, safe_stage: int) -> list[dict]:
    base = {
        key: value
...
def _rows() -> dict[str, list[dict]]:
    unsafe_early = _proposal(2, "c", stage=2, safe=False, context="terminal", commit_index=1)
    unsafe_late = _proposal(2, "b", stage=4, safe=False, context="yoke", commit_index=2)
    unsafe_regression = _proposal(0, "a", stage=1, safe=False, context="yoke", commit_index=1)
...
def test_type7_and_ecdf_are_exact() -> None:
    ...
    assert exact_ecdf([2, 1, 2]) == [
        {"value": 1.0, "count": 1, "cumulative_count": 1, "cumulative_fraction": 1 / 3, "denominator": 3},
        {"value": 2.0, "count": 2, "cumulative_count": 3, "cumulative_fraction": 1.0, "denominator": 3},
    ]


def test_complete_shot_bootstrap_is_deterministic_and_handles_zero_denominator() -> None:
...
def test_loader_analysis_reconciliation_ratio_of_sums_and_first_conflict(tmp_path: Path) -> None:
    ...
    assert {tuple((row["first_unsafe_stage"], row["first_unsafe_context"], row["count"])) for row in first} == {
        (1, "yoke", 1),
        (2, "terminal", 1),
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda rows: (
                rows[1].__setitem__("proposal_signature", rows[0]["proposal_signature"]),
                rows[1].__setitem__("stage", rows[0]["stage"]),
                rows[1].__setitem__("ordered_endpoints", rows[0]["ordered_endpoints"]),
                rows[1].__setitem__("canonical_edge_ids", rows[0]["canonical_edge_ids"]),
            ),
            "repeats a proposal signature",
        ),
        (lambda rows: rows[1].__setitem__("local_active_state_fingerprint", [99]), "changed"),
        (lambda rows: rows[3].__setitem__("stage", 1), "earlier stage"),
        (lambda rows: rows[0].__setitem__("terminal_action", rows[1]["terminal_action"]), "only on the final"),
        (lambda rows: rows[0].__setitem__("exhaustion_kind", "proposal"), "claims proposal exhaustion"),
    ],
)
...
def test_required_tail_telemetry_is_probe_fatal_but_smoke_explicitly_incomplete(
    ...
    assert (
        analysis["tables"]["veto_chain_tails"]
        ["stage3_enumeration_ns_per_state"]["telemetry_status"]
        == "incomplete-smoke"
    )
...
def test_gate7_is_missing_without_exact_authenticated_collector_attestation(
    ...
    with pytest.raises(PolicyAnalysisError, match="missing or unauthenticated fatal gate"):
        write_policy_analysis(unattested, analysis, render_plots=False)


def test_analysis_is_input_order_invariant_and_uses_trajectory_order(tmp_path: Path) -> None:
    root = tmp_path / "corpus"
    _write_corpus(root)
    corpus = load_policy_audit(root)
    ...
    assert reordered["tables"]["first_conflict_discordant"] == baseline["tables"]["first_conflict_discordant"]
    ...
    assert analyze_policy_audit(with_casebook)["analysis_sha256"] == baseline["analysis_sha256"]
...
def test_casebook_selection_is_deterministic_outcome_blind_and_sparse_flagged() -> None:
    ...
    changed = [dict(state, actual_observables="changed", logical_failure=not state["logical_failure"]) for state in states]
...
def test_atomic_writer_emits_tables_plot_data_and_analysis_ready(
    ...
    with pytest.raises(PolicyAnalysisError, match="COMPLETE is not an analysis substitute"):
        verify_existing_policy_analysis(loaded_again, recomputed)
