...
def test_tango_fixed_tables(table: PairedContingency, expected: float):
    assert tango_paired_risk_difference_upper(table) == pytest.approx(expected, abs=1e-11)
...
def test_counter_seed_matches_literal_sha256_rule():
    ...
    assert derive_stim_batch_seed(seed_root="00" * 32, batch_id=1) == 1_582_663_946_662_125_067
...
def test_batch_schedule_rejects_duplicates_gaps_overlaps_and_wrong_total():
    ...
    with pytest.raises(ValueError, match="duplicate"):
        validate_batch_schedule(
            [BatchSpec(1, 0, 10_000), BatchSpec(1, 10_000, 1)]
        )
...
def test_paired_workload_ratio_and_seeded_bootstrap():
    ...
    assert paired_workload_ratio(
        original_events=original, residual_events=residual
    ) == 0.5
